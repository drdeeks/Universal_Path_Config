@echo off
echo Updating Universal Path Config Portable Executable...
echo.
...
if not exist "dist\Universal Path Config 1.2.0 Portable.exe" (
    echo ERROR: New executable not found in dist folder.
    echo File should be at: "%CD%\dist\Universal Path Config 1.2.0 Portable.exe"
    echo.
...
taskkill /f /im "Universal Path Config 1.2.0 Portable.exe" 2>nul
taskkill /f /im "Universal Path Config 1.0.0 Portable.exe" 2>nul
taskkill /f /im "Universal Path Config.exe" 2>nul
timeout /t 3 /nobreak >nul

REM Remove old executable if it exists
echo Removing old executable...
if exist "Universal Path Config 1.2.0 Portable.exe" (
    echo Found existing executable, removing it...
    del /f "Universal Path Config 1.2.0 Portable.exe"
)
if exist "Universal Path Config 1.0.0 Portable.exe" (
    echo Found old v1.0.0 executable, removing it...
    del /f "Universal Path Config 1.0.0 Portable.exe"
    if errorlevel 1 (
...
echo Source: "%CD%\dist\Universal Path Config 1.2.0 Portable.exe"
echo Target: "%CD%\Universal Path Config 1.2.0 Portable.exe"

copy "dist\Universal Path Config 1.2.0 Portable.exe" "." >nul
if errorlevel 1 (
...
if exist "Universal Path Config 1.2.0 Portable.exe" (
    ...
    for %%A in ("Universal Path Config 1.2.0 Portable.exe") do echo New executable size: %%~zA bytes
    for %%A in ("dist\Universal Path Config 1.2.0 Portable.exe") do echo Source executable size: %%~zA bytes
) else (
...
echo File location: "%CD%\Universal Path Config 1.2.0 Portable.exe"
pause 